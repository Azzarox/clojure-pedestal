...
(defn- log-startup [cm]
  (log/info :msg "Full-stack server started" :port (:port cm))
  cm)
...
(defn connector-map
  ...
  (-> (connector/default-connector-map 8080)
      ...
      (connector/with-routes
        ;; resource-routes serves static files from resources/public/:
        ;;   GET /js/app.js   → resources/public/js/app.js
        ;;   GET /css/app.css → resources/public/css/app.css
        ;; These are matched BEFORE the /* catch-all in routes.clj
